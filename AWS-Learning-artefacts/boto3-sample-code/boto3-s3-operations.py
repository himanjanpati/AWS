...
def upload_file_re(file_path, object_name=None):
    if object_name is None:
        object_name = os.path.basename(file_path)
    s3_re = boto3.resource('s3')
    result = s3_re.Bucket('aws-s3-bucket-iam').upload_file(file_path, object_name)
    print(result)


upload_file_re("/Volumes/CodeSpace/AWS/AWS-Learning-artefacts/S3/test.json")
